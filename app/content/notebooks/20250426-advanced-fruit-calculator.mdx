...
slug: "20250426-advanced-fruit-calculator"
...
```python
cost = (apples_bought * apple_price) + (bananas_bought *
 banana_price) + (durians_bought * durian_price)
```
# Apply tax (9%)
```python
tax_rate = 0.09 tax_amount = cost * tax_rate total_cost = cost +
```
 tax_amount
...
# Print results
```python
print(f"Free apples: {free_apples}") print(f"Total cost after
 tax: ${total_cost:.2f}") print(f"Average price per fruit:
    ${average_price:.2f}") print(f"Durian cost split: ${dollar_part}
    + {cent_part*100:.0f} cents") print(f"Bananas to the power of 2:
```
    {banana_power}")
